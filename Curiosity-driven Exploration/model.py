...
class ICM(nn.Module):
    ...
    def forward(self,inputs):
        state, action, _, s_prime, _ = inputs
        ...
        # residual
        for i in range(4):
            pred_next_state_feature = self.residual(torch.cat((pred_next_state_feature_orig, action), 1))[i * 2]
            pred_next_state_feature_orig = self.residual(
                torch.cat((pred_next_state_feature, action), 1))[i * 2 + 1] + pred_next_state_feature_orig

        pred_next_state_feature = self.forward_net2(torch.cat((pred_next_state_feature_orig, action), 1))
